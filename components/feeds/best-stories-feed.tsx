...
export const BestStoriesFeed: FC = () => {
  ...
  useEffect(() => {
    const fetchBestStoryIds = async () => {
      const { data } = await axios.get(`${API_URL}/beststories.json`);
  ...
  return (
    <div className="grid grid-flow-row gap-2">
      ...
      <div className="grid place-items-center py-4">
        <button onClick={() => setSkip(prevSkip => prevSkip + fetchAmount)} className="bg-indigo-500 hover:bg-indigo-700 duration-300 ease-in-out p-1 rounded-md">
